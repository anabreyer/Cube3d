...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/23 14:36:41 by jischoi           #+#    #+#             */
/*   Updated: 2023/05/23 14:36:43 by jischoi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_w(t_player *player, t_map *map)
{
	...
	if (map->map[(int)(player->pos_y)] \
		[(int)(player->pos_x + vector_x * wall_dist)] == '0')
		player->pos_x += vector_x;
	if (map->map[(int)(player->pos_y + vector_y * wall_dist)] \
		[(int)(player->pos_x)] == '0')
...
void	move_d(t_player *player, t_map *map)
{
	...
	vector_y = player->dir_y * player->speed_move;
	if (map->map[(int)(player->pos_y)] \
		[(int)(player->pos_x + vector_y * wall_dist)] == '0')
		player->pos_x += vector_y;
	if (map->map[(int)(player->pos_y - vector_x * wall_dist)] \
		[(int)(player->pos_x)] == '0')
